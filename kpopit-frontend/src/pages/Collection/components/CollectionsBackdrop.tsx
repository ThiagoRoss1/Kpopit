...
// Stars SVG paths:  5-point outline, 4-point outline,
...
// Neon Glows
const GLOWS: { day: string; night: string }[] = [
    {
        day: 'radial-gradient(72% 62% at 6% -16%, rgba(255,51,153,0.11), transparent 60%)',
        night: 'radial-gradient(72% 60% at 6% -16%, rgba(255,51,153,0.13), transparent 62%)',
    },
    {
...
const SPARKLES: { color: string; className: string; tilt: number; delay: number }[] = [
    ...
    { color: '#ff3399', className: 'right-[18%] top-[30%] size-4', tilt: 12, delay: 1.4 },
    { color: '#ff3399', className: 'bottom-[26%] left-[9%] size-5', tilt: 6, delay: 2.6 },
    { color: '#ff3399', className: 'bottom-[18%] right-[16%] size-3.5', tilt: -12, delay: 0.8 },
...
function SheetTexture({ night }: { night: boolean }) {
    ...
    // Only remove the paper texture, avoiding to remove the night halftone, when the backdrop effect is off.
    if (!on) return null;
    ...
    return (
        <>
            ...
            {GLOWS.map((glow, index) => (
                <div key={index} className="absolute inset-0" style={{ background: night ? glow.night : glow.day }} />
            ))}
