        ...
        <AlbumContentShell groupName={group.group_name} ramp={ramp} side={side}>
            <div className="relative flex h-full flex-col px-7.5 pt-7.5">
                ...
                <div className="flex items-start justify-between">
                    <div className="flex flex-col whitespace-nowrap text-black [text-shadow:0.5px_0.5px_2px_rgba(0,0,0,0.35)]">
                        <p className="font-major-mono-display text-[22px] leading-[normal]">WE ARE</p>
                        <p className={`font-major-mono-display leading-[normal] ${group.group_name.length > 8 ? 'text-[38px]' : 'text-[52px]'}`}>
                ...
                <div className="flex flex-1 flex-col justify-center items-center px-5 pb-22">
                    ...
                    {photoUnlocked ? (
                        ...
                        <AlbumLockedGroupPhoto groupName={group.group_name} className="z-20 h-62.5 w-full" />
                    )}
                    {/* album progress */}
                    <div className="mt-3 w-full">
                        <div className="flex items-center justify-between">
                            <p className="font-major-mono-display text-[22px] leading-[normal] text-(--a2)">ALBUM PROGRESS</p>
                            <p className="font-major-mono-display text-[22px] leading-[normal] text-white [text-shadow:0.5px_0.5px_2px_rgba(0,0,0,0.5)]">
                                {owned}/{total}
                            </p>
                        </div>
                        <div
                    ...
                    <div className="mt-18.5 w-full overflow-clip rounded-br-[20px] rounded-tl-[20px] bg-white drop-shadow-[2px_4px_2px_rgba(0,0,0,0.4)]">
                        <div className="flex h-8.5 items-center justify-between bg-(--a0) px-5">
                            <p className="font-[DynaPuff] text-[22px] leading-[normal] text-white [text-shadow:0.5px_0.5px_2px_rgba(0,0,0,0.5)]">GROUP FILE</p>
                            <p className="font-[DynaPuff] text-[22px] leading-[normal] text-white [text-shadow:0.5px_0.5px_2px_rgba(0,0,0,0.5)]">
                                {owned}/{total}
                            </p>
                        </div>
                        <div className="grid grid-cols-2">
                            {fileCells.map(([label, value], i) => (
                                <div
                                    key={label}
                                    className={`flex items-center justify-between px-5 py-2 font-[DynaPuff] text-[16px] leading-[normal] text-black [text-shadow:0.5px_0.5px_2px_rgba(0,0,0,0.25)] ${i % 2 === 0 ? 'border-r border-[#cdcdcd]' : ''} ${i < 4 ? 'border-b border-[#cdcdcd]' : ''}`}
                                >
                                    <span>{label}</span>
                                    <span>{value}</span>
                                </div>
