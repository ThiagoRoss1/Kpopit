...
const Modal = (props: ModalProps) => {
    const { isOpen, onClose, children, title, isTransferDataSubPages, returnPage } = props;
    ...
    return (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-black/30 backdrop-blur-xs sm:backdrop-blur-sm" onClick={() => onClose()}>        
            <motion.div 
                initial={{ opacity: 0, scale: 0.8, y: 50 }}
                animate={{ opacity: 1, scale: 1, y: 0 }}
                exit={{ opacity: 0, scale: 0.8, y: 50 }}
                transition={{ duration: 0.2, ease: "easeInOut" }} 
                className="flex items-start justify-center w-full sm:max-w-211.5 mx-auto mt-20 mb-10" onClick={(e) => e.stopPropagation()}>

                <div className={`relative w-full max-w-211.5 text-center bg-black/80 border border-white/80 rounded-[20px] overflow-hidden shadow-[4px_4px_4px_1px_rgba(0,0,0,0.1),inset_0_4px_4px_rgba(0,0,0,0.25)]`}>
                    ...
                    <div className={`w-full flex items-center ${isTransferDataSubPages ? "justify-start gap-1 px-2" : "justify-between px-4"} py-2 bg-white/10 border-b border-white/20`}>
                        ...
                        <h2 className="max-xxs:text-2xl xxs:text-3xl sm:text-4xl font-bold bg-linear-to-b from-[#b43777] to-[#ec4850] bg-clip-text text-transparent drop-shadow-lg hover:cursor-default">
                            {title}
                        </h2>
                            <div className={`${isTransferDataSubPages ? "justify-end flex-1 px-2" : "justify-center"} items-center flex`}>
                            <button className="flex items-center justify-center rounded-full shrink-0 w-9 h-9 sm:w-10 sm:h-10 text-center
                            bg-linear-to-b from-[#b43777]/0 to-[#ec4850]/0 shadow-lg hover:brightness-115 hover:scale-105
